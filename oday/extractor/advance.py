...
from oday.common.colors import mc, nc, bw
    ...
    def fetch_dbs(
        ...
        if not retval.ok:
            if backend == "Microsoft SQL Server":
                logger.debug(
                    "oday could not determine number of databases, using DB_NAME to fetch dbs .."
                )
                ...
                if not payload:
                    logger.critical(
                        "oday was not able identify payload for database(s) fetching, try manually."
                    )
                    return _temp
    ...
    def fetch_tables(
        ...
        if retval.ok:
            ...
            if total > 0:
                ...
                if not payload:
                    logger.critical(
                        "oday was not able identify payload for table(s) fetching, try manually."
                    )
                    return _temp
                payload = clean_up_offset_payload(payload, backend=backend)
                if start == 0 and backend == "Oracle":
    ...
    def fetch_columns(
        ...
        if retval.ok:
            ...
            if total > 0:
                ...
                if not payload:
                    logger.critical(
                        "oday was not able identify payload for column(s) fetching, try manually."
                    )
                    return _temp
                payload = clean_up_offset_payload(payload, backend=backend)
                if start == 0 and backend == "Oracle":
    ...
    def dump_table(
        ...
        if retval.ok:
            ...
            if total > 0:
                ...
                if not payload:
                    logger.critical(
                        "oday was not able identify payload for table dump, try manually."
                    )
                    return _temp
                ...
                if backend == "Microsoft SQL Server":
                    ...
                    if payload.endswith("WHERE 1=1)"):
                        logger.warning(
                            "it was not possible to dump all of the entries for the SQL query provided. oday will assume that it returns only one entry"
                        )
                        start = 1
                        stop = 2
                if start == 0 and backend == "Oracle":
