...
from oday.core.inject import inject_expression
from oday.common.colors import black, white, DIM, BRIGHT
...
class OdayExtractor:
    ...
    def _check_operator(
        ...
        if conf.fetch_using and conf.fetch_using in list(operators.keys()):
            ...
            if operator:
                logger.debug(
                    f"oday will based data retrieval using '{conf.fetch_using}' openator"
                )
        ...
        while start < end:
            ...
            if retry_on_error >= conf.retry:
                start += 1
                # logger.warning(f"oday detected connection errors multiple times")
                # choice = logger.read_input(
            ...
            try:
                ...
                #     logger.critical(
                #         f"{attack.error_msg} HTTP error code detected. oday is going to retry."
                #     )
                #     time.sleep(0.5)
                ...
                if attack01 and vector_type == "boolean_vector":
                    ...
                    if result:
                        ...
                        if _type == "between_based_search":
                            msg = "it appears that the character '>' is filtered by the back-end server. oday will based data retrieval on BETWEEN operator"
                            if conf.fetch_using:
                                msg = ""
                            _temp = GuessUsing(
                                ok=True,
                        ...
                        if _type == "in_based_search":
                            msg = (
                                "it appears that the character '>' and 'BETWEEN' operator is filtered by the back-end server. oday will based data retrieval on IN() operator",
                            )
                            if conf.fetch_using:
                                msg = ""
                            _temp = GuessUsing(
                        ...
                        if _type == "linear_search":
                            msg = (
                                "it appears that the character '>' and the operator(s) 'IN, BETWEEN' are filtered by the back-end server. oday will based data retrieval on '=' operator, You are advised to use --delay=3 in this case",
                            )
                            if conf.fetch_using:
                                msg = ""
                            _temp = GuessUsing(
                ...
                logger.critical(
                    f"connection attempt to the target URL was aborted by the peer, oday is going to retry"
                )
                retry_on_error += 1
            except ConnectionRefusedError as e:
                logger.critical(
                    f"connection attempt to the target URL was refused by the peer. oday is going to retry"
                )
                retry_on_error += 1
            except ConnectionResetError as e:
                logger.critical(
                    f"connection attempt to the target URL was reset by the peer. oday is going to retry"
                )
                retry_on_error += 1
            except Exception as error:
                logger.critical(
                    f"error {error}, during operator check phase. oday is going to retry"
                )
                retry_on_error += 1
        if _temp.ok:
            if _temp.msg:
        ...
        if identified_character:
            for i in range(1, retry + 1):
                ...
                try:
                    ...
                    logger.critical(
                        f"error {error}, during detection phase. oday is going to retry"
                    )
                    retry_on_error += 1
        return is_valid
            ...
            if conf._readtimout_counter >= 3:
                if conf.rto_warning:
                    if not conf.rtom_warning:
                        choice = logger.read_input(
                            "oday detected read timeout multiple time(s). Do you want to continue? [y/N] "
                        )
                        if choice == "n":
                            logger.end("ending")
                            exit(0)
                ...
                if not conf.rto_warning:
                    ...
                    logger.warning(
                        f"oday detected read timout '{conf._readtimout_counter}' time(s){msgrto}."
                    )
    ...
    def fetch_length(
        ...
        if noc < 1:
            logger.debug(
                "oday couldn't determine the number if character(s) in length query"
            )
        length = 0
        if noc >= 1:
                    ...
                    if retval != "<blank_value>":
                        if backend == "Microsoft SQL Server":
                            if entry.endswith("sysobjects)") or entry.endswith(
                                ...
                                logger.warning(
                                    "it was not possible to count the number of entries for the SQL query provided. oday will assume that it returns only one entry"
                                )
                    try:
                        if dump_type and not conf.fresh_queries:
                            session.dump(
    ...
    def fetch_characters(
        ...
        if not retval_error.ok:
            ...
            # if other_vectors:
            #     logger.debug(
            #         "oday is going to use other injected vectors payloads if any."
            #     )
        ...
        for vector_type, vector in conf.vectors.items():
            ...
            if not retval_check.ok:
                logger.critical(
                    "oday will not be able to extract data as '=', 'IN' and '>' all are filtered by back-end server.."
                )
                logger.end("ending")
                exit(0)
                ...
                for _, value in entries.items():
                    ...
                    for entry in payloads:
                        ...
                        if (
                            ...
                            logger.debug(
                                "oday will use a fallback leaner search to guess character(s), adjusting threads to 1"
                            )
                            conf.threads = None
                        if conf.threads and vector_type == "boolean_vector":
                            ...
                            if conf.threads > conf._max_threads:
                                ...
                                if not conf.max_threads_warning:
                                    logger.warning(
                                        f"oday recommends using threads upto {conf._max_threads}. adjusting '--threads=\"{conf._max_threads}\"'."
                                    )
            ...
            if not is_done_with_vector:
                logger.debug(
                    f"oday was not able to extract the data with vector type '{vector_type}', switching to other vector type(s) if any.."
                )
