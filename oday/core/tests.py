...
from oday.common.colors import nc, mc
...
from oday.dbms.fingerprint import FingerPrintDBMS
    ...
    try:
        ...
        if retval:
            ...
            if param_tobe_tested in params_tested_already:
                ...
                logger.debug("oday is going to resume target exploitation.")
            is_resumed = True
        if not is_resumed:
            ...
            # for oday to consider content length based boolean injection types, default check is true
            ...
            if is_stable:
                ...
                is_dynamic = True
                warnMsg = "target URL content is not stable (i.e. content differs). oday will base the page "
        ...
        logger.critical(
            f"oday was not able to establish connection. try checking with -v set to 5. error '{error}' occured."
        )
        logger.end("ending")
        exit(0)
    if not is_resumed:
            ...
            if retval.possible_dbms:
                ...
                if not conf.test_filter:
                    ...
                    if _tech and not conf.prioritize:
                        logger.debug(
                            f"oday is going ask to set {_tech} as heuristic (basic) detected a possible DBMS '{_possible_dbms}' from SQL error message"
                        )
    ...
    for entry in blind_payloads:
        ...
        if possible_dbms or dbms:
            if entry.dbms and entry.dbms not in [possible_dbms, dbms]:
                ...
                continue
        logger.info(f"testing '{entry.title}'") #responsable to show testing and payloads on screen
        while index_of_payload < total_payloads:
            ...
            if attack.status_code in [403, 406]:
                logger.debug(
                    f"{attack.error_msg} HTTP error codes detected. oday is going to retry in few seconds.."
                )
                try:
            ...
            if retval:
                ...
                if not possible_dbms and not dbms:
                    ...
                    if not response_dbms:
                        logger.debug(
                            "oday could not determine the backend DBMS, detected payload is false positive, performing further tests.."
                        )
                        logger.warning(
                            "false positive payload detected, oday will perform further test(s) if any to ensure if target is injectable.."
                        )
                        continue
                    if backend:
                        choice = logger.read_input(
...
def get_injectable_payloads(
    ...
    if not injected_and_vulnerable:
        message = (
            "oday resumed the following injection point(s) from stored session:\n"
        )
    else:
        # show report for injection type,title,payloads
        message = "oday identified the following injection point(s) with a total of {nor} HTTP(s) requests:\n".format(
...
def check_session(
    ...
    if retval:
        ...
        if not bool(_temp):
            logger.critical(
                f"all tested parameters do not appear to be injectable., please rerun oday with '--flush-session'."
            )
            logger.end("ending")
            exit(0)
        ...
        if response:
            ...
            if boolean_vector and not error_based_in_vectors:
                ...
                if not backend:
                    ...
                    logger.warning("oday could not determine the backend DBMS")
                    logger.warning(
...
def check_injections(
    ...
    if is_injected:
        ...
        if is_injected_error:
            ms = f"oday identified that the parameter '{param_name}' is injectable with error-based"
            if is_injected_bool:
                ms += ", boolean based"
