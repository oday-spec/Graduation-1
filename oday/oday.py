...
from oday.extractor.common import target
from oday.extractor.advance import target_adv
from oday.core.extract import oday_extractor
from oday.logger.colored_logger import logger, set_level
from oday.core.tests import basic_check, check_injections
from oday.core.extract import oday_extractor as ge
...
def perform_injection(
    ...
    OdayResponse = collections.namedtuple(
        "OdayResponse",
        [
            "url",
            "data",
    ...
    if conf.safe_chars:
        logger.debug(
            f'oday is going to skip urlencoding for provided safe character(s): "{safe_chars}"'
        )
            ...
            for parameter in parameters:
                    ...
                    if not conf.test_filter:
                        if conf.prioritize and not conf._is_asked_for_priority:
                            ...
                            choice_priority = logger.read_input(
                                f"it is suggested to set '--technique=E{techniques.upper()}'. Do you want oday set it for you ? [Y/n] ",
                                batch=batch,
                                user_input="Y",
                            )
                ...
                if retval and retval.vulnerable:
                    ...
                    if backend == "Microsoft Access":
                        logger.warning(
                            "oday currently only supports DBMS fingerprint payloads for Microsoft Access, exfiltration will be added soon"
                        )
                        logger.info(
...
class Oday:
    ...
    def dump_current_db(
        ...
        logger.warning(
            "missing database parameter. oday is going to use the current database to enumerate table(s) entries"
        )
        if not current_db:
        ...
        if current_db:
            ...
            logger.error(
                "oday is expecting database name to enumerate table(s) entries."
            )
