...
#                           |                                         /                                     
# Design & Implementation SQL Injection Scanner .
# The Project for Education purpose only :).
# Final Year Software Engineering at SPU .

import oday
import argparse
from oday.common import banner
from oday.common.utils import dbms_full_name
from oday.logger.colored_logger import logger

def main():
    examples = "%(prog)s -u http://www.example.com/vuln.php?id=1 --dbs\n\n"
    
    # description = "Desgin by Oday & Omar."
    ...
    optimization = parser.add_argument_group(
        "Optimization",
        description="These options can be used to optimize the performance of oday",
    )
